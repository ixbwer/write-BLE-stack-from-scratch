...
int main(void)
{
	printk("\n");
	printk("===================================================\n");
	printk("  BLE PHY LE Encryption Demo\n");
	printk("  AES-ECB (SK derivation) + AES-CCM (PDU encrypt)\n");
	printk("  LTK = all zeros (demo), TERMINATE after %ds\n",
	       SLAVE_TERMINATE_DELAY_S);
	printk("===================================================\n");

	/* Preset LTK (all zeros for the demo).
	 * In production, the LTK comes from SMP pairing and is obtained through HCI. */
